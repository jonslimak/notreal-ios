...
- Current status: Stage 0 onboarding in progress; setup complete through `S0-T5`, with `S0-T6` started and paused on App Store Connect API credential verification in RevenueCat.
- Last updated: 2026-02-11
- Session closeout: paused during `S0-T6`; next step is to finish App Store Connect API credential wiring in RevenueCat, verify product status, then complete offering/package mapping.
...
| S0-T3 | Stage 0 | build | Configure monthly/yearly IAP products and sandbox tester accounts. | done | Created subscription group `notreal Premium`; added products `com.notreal.iosapp.dev.monthly` (49.99) and `com.notreal.iosapp.dev.yearly` (199.99); created 2 sandbox tester accounts; validated products, prices, and testers in App Store Connect on 2026-02-11. |
...
| S0-T6 | Stage 0 | build | Configure RevenueCat project/app, products, offerings, and entitlement mapping. | blocked | Started setup: created RevenueCat project `notreal`, added apps `notreal-dev` (`com.notreal.iosapp.dev`) and `notreal-prod` (`com.notreal.iosapp`), created entitlement `premium`, and added dev/prod monthly+yearly products. Current blocker: product status shows `Could not check` / `Connection issue` (App Store Connect API credentials still need verification in RevenueCat for both apps; then finish offerings/packages mapping). |
...
| S1-T3 | Stage 1 | build | Implement Firebase read layer for core models and remote config fetch. | not_started | |
| S1-T4 | Stage 1 | build | Add analytics dispatcher scaffold (without full event wiring yet). | not_started | |
...
| ID | Stage | Type | Task | Status | Evidence |
|---|---|---|---|---|---|
| S4-T1 | Stage 4 | build | Integrate RevenueCat purchase/restore flows for monthly/yearly plans. | not_started | |
...
| S5-Q2 | Stage 5 | test | Technical check: required events appear in BigQuery `dev` export. | not_started | |
